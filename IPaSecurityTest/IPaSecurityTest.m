...
-(void)testAES
{
    ...
    for (NSNumber* testMode in testCaseList) {
        ...
        for (NSDictionary* testCase in testList) {
            ...
            NSData *cipherData = [plainText encryptWithAlgorighm:kCCAlgorithmAES128 mode:mode padding:NO iv:iv key:key];
            ...
            NSData *decryptData = [cipherData decryptWithAlgorighm:kCCAlgorithmAES128 mode:mode padding:NO iv:iv key:key];
...
-(void)testDES2
{
    ...
    NSData *encryptData = [data encryptWithAlgorighm:kCCAlgorithmDES mode:kCCModeECB padding:YES iv:nil key:[NSData dataFromHexString:@"38627974656B6579"]];
    
    
    
    data = [encryptData decryptWithAlgorighm:kCCAlgorithmDES mode:kCCModeECB padding:YES iv:nil key:[NSData dataFromHexString:@"38627974656B6579"]];
...
-(void)testDES
{
    ...
    for (NSDictionary* testCase in testCaseList) {
        ...
        NSData *key = [NSData dataFromHexString:testCase[@"Key"]];
        NSData *realCipherData = [NSData dataFromHexString:testCase[@"realCipherData"]];
        NSData *cipherData = [plainText encryptWithAlgorighm:kCCAlgorithmDES key:key];
        ...
        NSData *decryptData = [cipherData decryptWithAlgorighm:kCCAlgorithmDES key:key];
        if (![decryptData isEqualToData:plainText]) {
            NSLog(@"decrypt data:%@",decryptData);
            NSLog(@"correct decrypt data:%@",plainText);
